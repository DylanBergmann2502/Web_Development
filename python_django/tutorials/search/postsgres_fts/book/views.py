...
def post_search(request):
    ...
    if 'q' in request.GET:
        ...
        if form.is_valid():
            ...
            # # D-weight : 0.1, C-weight : 0.2, B-weight : 0.4, A-weight : 1.0
            # from django.contrib.postgres.search import SearchQuery, SearchRank, SearchVector
            # vector = SearchVector('title', weight='A') + SearchVector('authors', weight='B')
            # query = SearchQuery(q)
            # # cover_density = the distance of matching terms are taken into account
            # # normalization: read postgres documentation
            ...
            # # default matching terms will be in bold. Add start_sel and stop_sel to remove this
            ...
            print("#2")
            print(
                Book.objects.filter(title__trigram_similar=q).annotate(similar=TrigramSimilarity('title', q)).order_by(
                    '-similar').explain(analyze=True))
